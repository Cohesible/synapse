// Adapted from:
// https://github.com/oven-sh/bun/blob/fd75ca7585554eefd3a201a275f8dce809690c9a/src/thread_pool.zig
...
pub const Batch = struct {
    ...
    pub fn pushTask(self: *Batch, task: *Task) void {
        return self.push(Batch.from(task));
    }
};
...
pub const default_thread_stack_size = brk: {
    ...
    if (@import("builtin").target.os.tag != .macos) break :brk default;
...
const Event = struct {
    ...
    noinline fn waitFor(self: *Event, timeout: usize) void {
        ...
        while (true) {
            ...
            // who will either exit on SHUTDOWN or acquire with WAITING again, ensuring all threads are awoken.
            // This unfortunately results in the last notify() or shutdown() doing an extra futex wake but that's fine.
            Futex.timedWait(&self.state, WAITING, timeout) catch {};
            state = self.state.load(.monotonic);
            acquire_with = WAITING;
        }
    }
